...
def handle_auto(channel_id):
    response = requests.get("https://bldcmprod-cdn.toffeelive.com/cdn/live/{channel_id}/playlist.m3u8", headers=headers)
...
def handle_single(path):
    ...
    myresponse = make_response(response.text.replace("/live/", "https://bldcmprod-cdn.toffeelive.com/live/"))
    myresponse.headers["Content-Type"] = "application/vnd.apple.mpegurl"
    return myresponse
